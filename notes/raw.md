...
The main thing I know is that the entire configuration of your OS is
in:
```
...
```
This comment writes `etc/nixos/configuration.nix` and `/etc/nixos/hardware-configuration.nix` (although
...
nixos-rebuild boot
```
Which compiles stuff. 

At this point, I no longer have wifi and I'm not sure how it all
...
in `/etc/nixos/configuration.nix`. There is a helpful search at
https://search.nixos.org . For example, I added:
```nix
